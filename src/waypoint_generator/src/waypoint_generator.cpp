...
void publish_waypoints() {
    ...
    pub1.publish(waypoints);//发布的点信息不包括起始点
    // cout << "enter publish_waypoints()" << endl;
    ...
    // pub2.publish(waypoints);//与publish_waypoints_vis()中pub2.publish(poseArray)功能一样，都是加上了odom中的起始点信息
    waypoints.poses.clear();
}

void publish_waypoints_vis() {
    //cout << " enter publish_waypoints_vis()" << endl;
...
/*    if (!is_odom_ready) {
    ...
    } else if (waypoint_type == string("manual-lonely-waypoint")) {//手动给定一个点(这个点是规划过程的最终点)
